...
class WatchdockClient {
  ...
  private cleanupAutoCapture: (() => void) | null = null;
  ...
  init(options: WatchdockInitOptions): void {
    ...
    if (typeof window === "undefined") {
      throw new Error("watchdock-errors-browser must run in a browser environment. Use watchdock-errors for Node.");
    }
  ...
  private setupAutoCapture(): void {
    const onError = (event: ErrorEvent) => {
      this.captureException(event.error ?? event.message, { title: "Unhandled error" });
    };

    const onUnhandledRejection = (event: PromiseRejectionEvent) => {
      this.captureException(event.reason, { title: "Unhandled promise rejection" });
    };
      ...
      let event: WatchdockEventPayload = sanitizeEvent(
        {
          ...
          server: buildServerPayload(state.appName, mergedContext.server),
          sdk: {
            name: SDK_NAME,
            version: SDK_VERSION,
          },
...
function sendInitPing(state: WatchdockClientState): void {
  setTimeout(() => {
    fetch(`${baseUrl(state.endpoint)}/api/v1/errors/sdk-init/`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${state.apiKey}`,
